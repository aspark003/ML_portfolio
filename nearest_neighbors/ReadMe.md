...
Key observations
Black dash (lambda_val) marks the extraction cutoff for a selected cluster; it is the representative density level used to extract that cluster.
...
Higher lambda values correspond to denser, more persistent structure.

Lower lambda values (near 0) usually indicate low persistence and noise.
